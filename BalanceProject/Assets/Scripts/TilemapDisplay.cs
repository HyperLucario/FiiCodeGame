...
{	
	...
	{
        int mapWidth = Global.mapWidth, mapHeight = Global.mapHeight;
        float[,] noiseMap = Global.HeightMap;
        float[,] featureMap = Global.FeatureMap;
        int[,] intHeightMap = new int[mapWidth, mapHeight];
		...
		for(int x = 0; x < mapWidth; x++){
			for(int y = 0; y < mapHeight; y++){
				...
				foreach(Transform child in transform)
				{
					...
					if(tmRenderer.sortingOrder == 1) continue;
